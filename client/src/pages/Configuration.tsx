...
function UserAccess({onInvite,isAdmin}:{onInvite:()=>void;isAdmin:boolean}){
  ...
  const update=useMutation({mutationFn:({id,...body}:{id:string;role?:string;active?:boolean})=>api(`/admin/users/${id}`,{method:'PATCH',body:JSON.stringify(body)}),onSuccess:()=>qc.invalidateQueries({queryKey:['admin-users']}),onError:error=>setActionError(error instanceof Error?error.message:'Unable to update access')});
  ...
  return <div className="max-w-5xl space-y-4">
    ...
    <div className="panel overflow-hidden"><div className="flex items-center border-b bg-slate-50 px-4 py-3"><div><h2 className="text-sm font-semibold">Users with access</h2><p className="text-xs text-slate-500">Change a role or suspend access immediately.</p></div>{isAdmin&&<Button className="ml-auto" onClick={onInvite}><UserPlus size={14}/>Invite person</Button>}</div><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-xs"><thead className="border-b text-slate-500"><tr><th className="p-3">User</th><th>Email</th><th>Role</th><th>Status</th></tr></thead><tbody>{users.data?.map(user=><tr className="border-b" key={user._id}><td className="p-3"><span className="flex items-center gap-2"><Avatar name={user.name}/><b>{user.name}</b></span></td><td>{user.email}</td><td><select className="field w-44" value={user.role?._id||''} disabled={update.isPending} onChange={event=>update.mutate({id:user._id,role:event.target.value})}>{roles.data?.map(role=><option key={role._id} value={role._id}>{role.name}</option>)}</select></td><td><button disabled={update.isPending} className={`badge ${user.active?'bg-emerald-100 text-emerald-700':'bg-slate-200 text-slate-600'}`} onClick={()=>update.mutate({id:user._id,active:!user.active})}>{user.active?'Active':'Suspended'}</button></td></tr>)}</tbody></table></div></div>
