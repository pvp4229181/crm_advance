...
export function Activities(){
  ...
  const filterGroups=[
    ...
    {key:'company',label:'Company',options:distinct(all.filter(activity=>activity.company).map(activity=>({value:activity.company!._id,label:activity.company!.name})))},
    {key:'campaign',label:'Campaign',options:distinct(all.filter(activity=>activity.campaign).map(activity=>({value:activity.campaign!._id,label:activity.campaign!.name})))},
  ];
...
export function Calendar(){
  ...
  const openActivity=(activity:Activity)=>{const related=activity.relatedId as unknown;const id=typeof related==='string'?related:related&&typeof related==='object'&&'_id' in related?String((related as {_id:string})._id):'';if(activity.relatedModel==='Opportunity'&&id)navigate(`/opportunities/${id}`);else if(activity.relatedModel==='Lead')navigate(id?`/leads/${id}`:'/leads');else if(activity.relatedModel==='Contact'||activity.relatedModel==='Company')navigate('/contacts');else navigate('/activities')};
